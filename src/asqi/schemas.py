...
class InputDatasetBase(BaseModel):
    """Common fields for all input dataset declarations in a manifest."""
    ...
    features: Optional[list[DatasetFeature]] = Field(
        None,
        description="List of required features (only applicable to HuggingFace datasets).",
    )


class HuggingFaceInputDataset(InputDatasetBase):
    """Input dataset that expects a structured HuggingFace dataset."""

    type: Literal[HF_DATASET_TYPE] = Field(
        HF_DATASET_TYPE,
        description="Dataset type for structured HuggingFace datasets.",
    )
    features: list[DatasetFeature] = Field(
        ...,
        min_length=1,
        description="Required features for the HuggingFace dataset.",
    )


class FileInputDataset(InputDatasetBase):
    """Input dataset that expects direct file loading (pdf/txt)."""

    type: Literal[AcceptedFileTypes.PDF, AcceptedFileTypes.TXT] = Field(
        ...,
        description="Dataset type for file-based inputs (pdf or txt).",
    )

# Discriminated union on the `type` field
InputDataset = Annotated[
    Union[HuggingFaceInputDataset, FileInputDataset], Field(discriminator="type")
]
